...
## 2026-09-24 - Roaming companion that handles routine requests, and the Agents panel sized to its controls

Linux cloud container (Node 22, Python 3.11), branch claude/gallant-babbage-lshjfb. The companion left the workspace's fixed track for renderer/companion.js (bundled after workspace.js); the workspace publishes mefi:companion-state and exposes MefiWorkspace.companion; nav.js skips the Decision needed toast for a card the companion is about to settle; Command's rail names its view and the Agents view hugs its content. npm run build-booklet ok; npm run check ok (110 targets, 246 specs, all selectors used); npm run audit 0 errors / 0 warnings; eslint on the touched files clean. New tests/companion.test.mjs 17/17 and the Agents sizing test in command_toolbar 10/10. npm run test:fast: 2602 tests, 2588 pass, 0 fail, 14 skipped. Python contracts: 248 OK (3 skipped). Electron lane under xvfb-run with ELECTRON_DISABLE_SANDBOX=1 (the container runs as root): 28 tests, 25 pass, 2 skip, 1 fail - command_render "__assistant__ retains its label position", which fails the same way twice on the stashed baseline, so it is this container, not the change; without the sandbox switch six suites stop at "Running as root without --no-sandbox" on baseline and change alike. Checked by eye in Chromium against a fake bridge at 1000x600, 1440x900, 1920x1080 and 2560x1440: the companion rests on its workspace spot, walks to the Needs you tile to speak, stands left of Command's rail, answered the routine retry card and left the permission card; the Agents rail went from 1260px to 814px tall at 2560x1440 and still scrolls inside at 1000x600.
