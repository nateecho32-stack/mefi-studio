...
## 2026-09-22 night - Projects rail toggle follow-up

The M+ Projects panel now treats its rail door as part of the hover and focus boundary so an open panel can close on a second click, including a held press; keyboard focus leaving both surfaces still dismisses it. The change uses synthetic UI data only. npm run build-booklet passed; focused sidebar tests passed 10/10 and the real Electron startup fixture passed 1/1 with open, hover, held-press and close assertions. npm run check and npm run audit passed with zero findings. Final npm test exited 0: parallel Node 2,219 pass / 0 fail / 3 skip, heavy Electron 17/17 pass, exclusive Electron 1/1 pass plus one capability-gated occlusion skip, Python 247 OK (1 skip), and all normalized-path checks passed. The running portable app was not rebuilt for this review.

## 2026-09-22 night - Home conversation real-layout follow-up

Follow-up to the evening UI field notes after read-only review. The overview heading now says "cards in this view" because completed cards may be folded. A new disposable Electron startup check loads 40 synthetic conversation messages before Home is visible and requires the visible scroller to reach the newest message. Its first run exposed a real 97 px shortfall, and an immediate font-settling retry still missed by 82 px. The first pin now waits until the startup gate releases; a later layout repin retains the reader's position if they have scrolled away. No live project data or workers are used by the fixture.

Final verification on this isolated worktree: node --test tests/workspace_ui.test.mjs tests/tasks_ui.test.mjs passed 67/67; startup_render and task_overview_render passed 1/1 each in real Electron; npm run build-booklet, npm run check, and npm run audit passed (zero audit findings). The final npm test passed with Electron enabled: parallel Node 2,218 pass / 0 fail / 3 skip, heavy Electron 17/17 pass, exclusive Electron 1/1 pass plus one opt-in skip, Python 247 OK (1 skip), and all normalized-path lock checks passed. Full output is at %TEMP%\mefi-ux-clarity-npm-test-final.log.

## 2026-09-22 evening - Home conversation and Task board count clarity

Hands-on review of the running portable app found two reproducible clarity issues. The Task board overview folded related work into cards while its filters counted individual saved tasks, with no unit labels. Home could show old conversation entries after the messages loaded while its layer was hidden. The renderer now labels cards, starts the first visible conversation at the latest message, and preserves manual history reading; docs/ux-field-notes-2026-09-22.md records the observations without local task data.

Focused Node suites: 67/67 pass (workspace_ui + tasks_ui). The first focused attempt had one failing new assertion because its fake layer was left visible; correcting that fixture made the hidden-to-visible regression pass. Booklet build passed; npm run check passed; npm run audit returned zero findings. The first npm test passed with Electron fixtures skipped because the isolated worktree had no node_modules. After linking the existing dependency installation, the full npm test passed with the Electron lane enabled: parallel Node 2,218 pass / 0 fail / 3 skip, heavy Electron 17/17 pass, exclusive Electron 1/1 pass plus one opt-in skip, Python 247 OK (1 skip), normalized-path lock checks all passed. Separate real Electron startup_render and task_overview_render fixtures also passed. The full output is saved locally at %TEMP%\mefi-ux-clarity-npm-test.log.
