...
// ===== style: sigil =====
  ...
  // and a hexagon sweeps out to 1.52. The cells fold back in, last first, as
  // the work eases off. Everything runs on the node's integrated clock, so a
  ...
  // base, tip, base, as x, y pairs; tip 0 on top.
  const SIGIL_CROWN = (() => {
  ...
  // sinks toward the theme's background (a pale parchment seal on a light
  // theme, a dark one on a dark theme) and the ink rises toward its
  // highlight; on a light theme the rim darkens a little and the glows use
  // the hue itself, so every state tint stays legible. The theme's second hue
  // is only ever a highlight (the tail rune, the lit cell, the crown, the
  // wire packets); without one it is the whitened tint. A flash lights the
  // seal in the whitened tint, or on paper in the hue itself. Colour strings
  // are all at alpha 1: every level is a globalAlpha gain.
  ...
  function sigilTones(tint, palette) {
    ...
    const edge = light ? mix(tint, hi, 0.45) : tint;
    const deep = mix(tint, bg, light ? 0.72 : 0.84);
    const hot = mix(tint, hi, 0.55);
    const ink = mix(tint, hi, light ? 0.9 : 0.88);
    const rune = mix(tint, hi, light ? 0.55 : 0.42);
    const accent = palette.accent2 ?? hot;
  ...
  function sigilWellPaint(ctx, tones) {
    ...
    well.addColorStop(0, rgba(tones.tint, 0.3));
    well.addColorStop(0.6, rgba(tones.tint, 0.1));
  ...
  // seal's edges along the edge normals and lock at 1.19. While the node
  // works each assembles on the work's age, .12 s after the one before it,
  // with a little overshoot; once it stops they fold back in on the eased
  // work level, the last one first. The scan's cell (and the one it just
  // left) wear the second hue, and at T3 the struck rune is written into the
  // lit cell, engraved in the body's tone. Drawn under the seal, so the cells
  // come out of it.
  function sigilCells(ctx, tones, detail, base, work, active, age, lit, flare, trail, px, rune, engrave) {
  ...
  function paintSigil(ctx, p, radius, tint, n, m) {
    ...
    if (lattice && work > 0.02 && detail >= 1) sigilCells(ctx, tones, detail, base, work, n.active, age, slot, flare, trail, px, (near + shape0) % 6, detail >= 3 ? tierIn(r, 3) : 0);
    ...
    ctx.globalAlpha = base * (0.85 + 0.15 * lit); ctx.strokeStyle = tones.edgeS;
    ...
    if (detail === 0) {
      ...
      if (tail > 0.01) {
        ...
        ctx.globalAlpha = base * tail; ctx.strokeStyle = tones.accentS; ctx.lineWidth = 1.2 * px; ctx.stroke();
      }
    ...
    // At work a hexagon sweeps out from the rim to 1.52 and fades.
  ...
  // The work orbit: a comet running round a flat-top hexagon at r + 9 (its
  // corners toward the cells, so its sides pass between them) in the node's
  // own tint (the theme's orbit hue without one), three pieces fading behind
  // its head, on the node's integrated orbit phase.
  function sigilOrbit(ctx, p, radius, tint, o) {
    const palette = o.theme ?? theme(null);
    const tones = sigilPaints(ctx, tint ?? palette.orbit, palette);
    const ring = sigilNum(o.ring, radius + 9);
  ...
  // Hover: a hexagon at 1.2 in the whitened tint, breathing a little (it
  // steps back while the cells are out, which carry their own mark, and
  // gives way to the crown on a chosen node at work). Chosen:
  // the six points of a hexagram at 1.3 in the theme's second hue, turning
  // once in 20 s; a small node keeps a second hexagon instead.
  function sigilSelect(ctx, p, radius, tint, o) {
    ...
    const work = clamp01(sigilNum(m?.work, o.active === true ? 1 : 0));
    const alpha = sigilNum(o.alpha, 1), detail = sigilNum(o.detail, 3);
    ctx.save();
    ctx.translate(p.x, p.y);
    ctx.lineCap = "round"; ctx.lineJoin = "round";
    const hover = (1.2 + (still ? 0 : 0.03 * Math.sin(TAU * sigilFrac(time / 1800 + seed)))) * radius;
    const gain = 0.75 * sel * (1 - (o.chosen === true ? 1 : 0.7) * work);
    if (gain > 0.01) {
      ctx.beginPath(); sigilHex(ctx, 0, 0, hover);
    ...
    if (o.chosen === true) {
      ...
      if (detail >= 2) {
        ctx.rotate(still ? 0 : TAU * sigilFrac(time / 20000 + seed));
        ...
        ctx.beginPath(); sigilHex(ctx, 0, 0, radius * 1.32);
        ctx.lineWidth = 1.4; ctx.stroke();
      }
    }
    ctx.restore();
  ...
  // Labels clear the seal at rest and the lattice and its sweep at work.
  const sigilReach = (m) => 1 + 0.55 * clamp01(sigilNum(m?.work, 0));
