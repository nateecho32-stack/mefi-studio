...
// ===== wires =====
  ...
  // between the parameter `inside` (the curve's end at that centre) and
  // `outside` (out of the circle): eight halvings, then the chord between
  // the last two. The curve's
  // speed at its end says nothing about where it leaves a circle (the tree's
  // S-curve leaves its node upright and slow, then swings wide), so the
  // crossing is found, not estimated.
  function curveCross(cx, cy, r, inside, outside) {
    let lo = inside, hi = outside;
    pathBlossom(PATH, hi, hi, hi, CROSS_AT);
    let far = Math.hypot(CROSS_AT.x - cx, CROSS_AT.y - cy), near = 0;
    if (!(far > r)) return outside;
    for (let step = 0; step < 8; step += 1) {
      const mid = (lo + hi) / 2;
      pathBlossom(PATH, mid, mid, mid, CROSS_AT);
      const d = Math.hypot(CROSS_AT.x - cx, CROSS_AT.y - cy);
      if (d < r) { lo = mid; near = d; } else { hi = mid; far = d; }
    }
    return lo + (hi - lo) * (far > near ? (r - near) / (far - near) : 1);
  }
  // The tree's S-curve (a cubic through the caller's controls) cut where it
  // leaves a's circle of radius rA and meets b's of radius rB, by its
  // blossoms, into TRIM_A, TRIM_CP and TRIM_B.
  function trimCurve(a, b, cp, rA, rB) {
    ...
    const t0 = rA > 0 ? curveCross(a.x, a.y, rA, 0, 1) : 0;
    const t1 = rB > 0 ? curveCross(b.x, b.y, rB, 1, t0) : 1;
  ...
  function wire(ctx, style, a, b, o) {
    ...
    if (cp && !Object.isFrozen(given)) {
      // On the S-curve, the curve's own piece between the two surfaces.
      trimCurve(a, b, cp, rA, rB);
      given.cp = TRIM_CP;
