...
| Keys | Two credential worlds: the Studio's DPAPI fields in `%APPDATA%\Mefi's Studio AI+\auth.json` and OpenCode's own `~/.local/share/opencode/auth.json` + env variables. Neither side reads the other. | `main.cjs:74`, `:10619-10625`; `opencode auth list` |
