...
Investigation record for task `task_idea_mucpv6bo_0` (no application code changed).
...
docs read on 2026-09-22 from `packages/coding-agent/docs/`. Nothing pi-related is
installed on this machine (no `~/.pi`, no global npm package), so every pi claim
below is sourced from the upstream docs, not local files. Pi's layout changes
across releases; re-verify before relying on details.
...
| `<agent-dir>/auth.json` | Saved credentials per provider: `{ "<provider>": { "type": "api_key", "key": "...", "env": { ... } } }`. Holds API keys and OAuth tokens from `/login`. Plaintext JSON — the docs only warn "keep it private", no OS-keystore encryption. |
...
| Project scope | `.pi/settings.json` after a trust gate; resources combine | Project overrides for agent settings; credentials and catalog stay app-wide |
...
`earendil-works/pi` @ `main` (2026-09-22).
