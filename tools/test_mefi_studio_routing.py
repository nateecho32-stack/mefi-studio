...
class MefiStudioRoutingTests(unittest.TestCase):
    ...
    def test_auto_setup_plans_from_saved_keys_and_installed_clis(self):
        ...
        self.assertIn('ipcMain.handle("settings:auto-setup", () => autoSetup())', self.main, "Settings' button rides the shared autoSetup callable")
        ...
        self.assertRegex(self.main, r"if \(!SMOKE && !CAPTURE && !CLI_MODE\)[^\n]*firstLaunchAutoSetup\(", "a fresh install's first interactive launch runs auto setup by itself")
        launch = _function_body(self.main, "firstLaunchAutoSetup")
        self.assertIn("firstLaunchNeedsSetup(settings)", launch, "only a pristine settings file triggers the automatic pass")
        self.assertIn('send("setup:auto-setup", record)', launch, "the renderer hears about the automatic pass")
        self.assertIn("onAutoSetup", self.preload, "preload exposes the first-launch announcement")
