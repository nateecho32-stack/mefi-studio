...
test("Project map explores files with restorable camera history, search, gestures and reduced motion", { skip: !canRun, timeout: 120000 }, async (t) => {
  const fixture = await mkdtemp(path.join(tmpdir(), "mefi-project-map-render-"));
  try {
    ...
    const env = { ...process.env, MEFI_PROJECT_MAP_RENDER_FIXTURE: fixture }; delete env.ELECTRON_RUN_AS_NODE;
    const child = spawn(executable, [path.join(studio, "tests", "fixtures", "project-map-render-electron.cjs")], { cwd: studio, env, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    ...
    const artifacts = process.env.MEFI_PROJECT_MAP_CAPTURE_DIR;
