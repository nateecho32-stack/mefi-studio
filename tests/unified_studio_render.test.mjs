...
test("Unified Agents navigation, stable companion controls and scrollbar-free responsive layouts", { skip: !canRun, timeout: 180000 }, async (t) => {
  ...
  try {
    ...
    const env = { ...process.env, MEFI_UNIFIED_RENDER_FIXTURE: fixture }; delete env.ELECTRON_RUN_AS_NODE;
    const child = spawn(executable, [path.join(studio, "tests", "fixtures", "unified-studio-render-electron.cjs")], { cwd: studio, env, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
    let output = "";
    ...
    const artifacts = process.env.MEFI_UNIFIED_CAPTURE_DIR;
