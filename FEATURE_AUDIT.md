...
| Context manager and usage tracker | Initial implementation | Task context preview prioritizes the current brief and obligations within an explicitly estimated token budget, listing included, shortened and omitted sources while preserving saved originals. It is not yet the universal worker context compiler. The usage tracker keeps per-day, per-provider and per-model totals with unknown counters and retained lifetime totals, and reads OpenCode Go's own 5-hour/weekly/monthly windows over the saved key on demand; the account reading and the local estimate are shown separately. External coding CLI sessions are not counted, and other providers' account balances are not read. |
