...
process.on("exit", () => {
  ...
  if (stylerChild) spawn("taskkill", ["/pid", String(stylerChild.pid), "/t", "/f"], { windowsHide: true });
  if (stylerSetupChild) spawn("taskkill", ["/pid", String(stylerSetupChild.pid), "/t", "/f"], { windowsHide: true });
  saveAssistantSync();
